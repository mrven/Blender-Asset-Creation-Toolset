...
# region Palette props
	save_dir: StringProperty(description="Save Directory")

	custom_save_path: BoolProperty(description="Custom Save Path")

	pbr_workflow: BoolProperty(description="PBR Workflow")

	save_path: StringProperty(description="Path for Save Palettes", subtype='DIR_PATH')
#endregion

#region Export props
	export_dir: StringProperty(description="Export Directory")

	fbx_export_mode: EnumProperty(items=EXPORT_MODE_ITEMS)

	export_format: EnumProperty(items=EXPORT_FORMAT_ITEMS)
	...
	gltf_export_image_format: EnumProperty(items=GLTF_IMAGE_FORMAT_ITEMS)
	...
	custom_fbx_name: StringProperty(description="Custom Name for FBX")
	...
	export_path: StringProperty(description="Path for Export FBX", subtype='DIR_PATH')
	...
	export_smoothing: EnumProperty(items=EXPORT_SMOOTHING_ITEMS)

	export_custom_props: BoolProperty(description="Custom Props", default=True)

	# Export FBX Target Engine
	export_target_engine: EnumProperty(items=EXPORT_TARGET_ENGINE_ITEMS)
	...
	export_vc_color_space: EnumProperty(items=EXPORT_VC_COLOR_SPACE_ITEMS)
	...
	# Custom Forward Axis
	custom_export_forward_axis: EnumProperty(default="-Z", items=EXPORT_AXIS_ITEMS, update=update_custom_forward_axis)

	# Custom Up Axis
	custom_export_up_axis: EnumProperty(default="Y", items=EXPORT_AXIS_ITEMS, update=update_custom_up_axis)
#endregion

#region Rename props
	delete_prev_nums: BoolProperty(description="Delete Previous Numbers from Object Names", default=True)

	nums_method: EnumProperty(items=NUMBERING_METHOD_ITEMS)

	nums_format: EnumProperty(items=NUMBERING_FORMAT_ITEMS)
...
#region UV tools props
	uv_move_factor: EnumProperty(items=UV_MOVE_FACTOR_ITEMS, default='3')

	uv_index_rename: IntProperty(description="UV Index", min=0, max=10)

	uv_name_rename: StringProperty(description="UV Name")

	uv_name_add: StringProperty(description="UV Name")

	uv_packing_mode: EnumProperty(items=UV_PACKING_ITEMS)
...
#region Other tools props
	...
	merge_bones_method: EnumProperty(items=MERGE_BONES_METHOD_ITEMS)

	# Add Collection name to objects name or Replace
	col_to_obj_name_method: EnumProperty(items=COL_TO_OBJ_NAME_METHOD_ITEMS)

	# Where Place the collection name
	col_name_position: EnumProperty(items=COL_NAME_POSITION_ITEMS)

	# Style of type's spelling (Mesh or MESH)

	col_name_type_style: EnumProperty(items=COL_NAME_STYLE_ITEMS)
