...
def export_model(path, name, fbx_scale_mode):
	...
	if act.export_custom_options:
		if act.export_format == 'FBX':
			bpy.ops.export_scene.fbx(filepath=str(path + name + '.fbx'), use_selection=True,
		...
		if act.export_format == 'OBJ':
			bpy.ops.export_scene.obj(filepath=str(path + name + '.obj'), use_selection=True, use_mesh_modifiers=True,
