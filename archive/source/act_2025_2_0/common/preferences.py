...
# endregion
	...
	def draw(self, _):
		...
		if self.show_default_uv:
			local_box = box.box()
			row = local_box.row(align=True)
			row.label(text="UV ID")
			row.prop(self, "uv_index_rename")
			row = local_box.row(align=True)
			row.label(text="Rename UV Name")
			row.prop(self, "uv_name_rename")
			local_box = box.box()
			row = local_box.row(align=True)
			row.label(text="Add UV Name")
			row.prop(self, "uv_name_add")
			row = local_box.row(align=True)
			row.label(text="Packing")
			row.prop(self, "uv_packing_mode")
			if self.uv_packing_mode == "SMART":
				row = local_box.row(align=True)
				row.label(text="    Angle")
				row.prop(self, "uv_packing_smart_angle")
				row = local_box.row(align=True)
				row.label(text="    Margin")
				row.prop(self, "uv_packing_smart_margin")
			if self.uv_packing_mode == "LIGHTMAP":
				row = local_box.row(align=True)
				row.label(text="    Quality")
				row.prop(self, "uv_packing_lightmap_quality")
				row = local_box.row(align=True)
				row.label(text="    Margin")
				row.prop(self, "uv_packing_lightmap_margin")
			local_box = box.box()
			row = local_box.row(align=True)
			row.label(text="UV Mover Step       1/")
			row.prop(self, "uv_move_factor")
		...
		if self.show_default_material:
			...
			row.prop(self, "palette_pbr_workflow")
			row = box.row(align=True)
			row.label(text="Custom Save Path")
			row.prop(self, "palette_custom_save_path")
			if self.palette_custom_save_path:
				row = box.row(align=True)
				row.label(text="    Save Path")
				row.prop(self, "palette_save_path")
		...
		if self.show_default_other:
			local_box = box.box()
			row = local_box.row(align=True)
			row.label(text="Merge Bones Method")
			row.prop(self, "merge_bones_method")
			local_box = box.box()
			row = local_box.row(align=True)
			row.label(text="Collection to Name Method")
			row.prop(self, "col_to_obj_name_method")
			if self.col_to_obj_name_method == "ADD":
				row = local_box.row(align=True)
				row.label(text="    Place")
				row.prop(self, "col_name_position", expand=False)
			else:
				row = local_box.row(align=True)
				row.label(text="    Style of Type")
				row.prop(self, "col_name_type_style", expand=False)
			local_box = box.box()
			row = local_box.row(align=True)
			row.label(text="Recalculate Normals")
			row.prop(self, "calc_normals_en")
			if self.calc_normals_en:
				row = local_box.row(align=True)
				row.label(text="    Inside")
				row.prop(self, "normals_inside")
			local_box = box.box()
			row = local_box.row(align=True)
			row.label(text="Cleanup: Delete empty meshes")
			row.prop(self, "delete_empty_meshes")
